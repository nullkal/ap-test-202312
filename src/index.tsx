...
const getSelfUser = async () => await prisma.user.findUnique({
...
app.post(`/users/${USERNAME}/inbox`, async (c) => {
  ...
  switch (body["@type"]) {
    case 'Follow': {
      ...
      const actor: any = await ky.get(actorId, { headers: { "Accept": "application/activity+json" } }).json()
      const actorUser = prisma.user.upsert({
        where: { domain_screenName: { domain: actorDomain, screenName: actor.preferredUsername } },
        create: {
  ...
  return c.html(<html>
    ...
    <body>
      <div style="display: flex; align-items: center;">
        <img src="/static/icon.png" width="64" hegiht="64" style=" width: 64px; height: 64px; border: solid 1px #999; flex-shrink: 0;" />
        <div>
          <p>
            @nullkal<br />
            フォロワー: <a href="/followers">{await prisma.follows.count({ where: { follower: selfUser } })}人</a>,
            フォロー中: <a href="/followings">{await prisma.follows.count({ where: { following: selfUser } })}人</a>
          </p>
        </div>
      </div>
